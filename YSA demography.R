...
yellow_pi1aInc <- mutate(yellow, pi = ifelse(stage == "1a", pi+(1-pi), pi *1))
yellow_pi1aInc <- mutate(yellow_pi1aInc, piSD = ifelse(stage == "1a", (piSD - piSD), piSD *1))
yellow_pi1bInc <- mutate(yellow, pi = ifelse(stage == "1b", pi+(1-pi), piSD *1))
yellow_pi1bInc <- mutate(yellow_pi1bInc, pi = ifelse(stage == "1b", (piSD- piSD), piSD *1))
yellow_pi1cInc <- mutate(yellow, pi = ifelse(stage == "1c", (0.999-pi+pi), pi *1))
yellow_pi1cInc <- mutate(yellow_pi1cInc, pi = ifelse(stage == "1c", (piSD - piSD), piSD *1))
yellow_pi2Inc <- mutate(yellow, pi = ifelse(stage == "2", (0.999-pi+pi), pi *1))
yellow_pi2Inc <- mutate(yellow_pi2Inc, pi = ifelse(stage == "2", (piSD - piSD), piSD *1))
yellow_pi3Inc <- mutate(yellow, pi = ifelse(stage == "3", (0.999-pi+pi), pi *1))
yellow_pi3Inc <- mutate(yellow_pi3Inc, pi = ifelse(stage == "3", (piSD - piSD), piSD *1))
#decreases 
...
graph1 <- graph + labs(x = "Stage Class", y = "Intrinsic rate of Increase (r)") 
graph2 <- graph1 + theme(panel.grid.minor=element_blank(), panel.grid.major=element_blank())
figure1a <- graph2 + theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1)) + scale_x_discrete(limits = class)
figure1a 
class <- c("fecundity", "egg", "nestling", "fledgling", "juvenile", "adult")
